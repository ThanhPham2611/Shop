...
export const get_info_shop = async (req, res) => {
  try {
    ...
    const followerInfo = await Follower.find(
      { followerId: shopInfoID.shopId },
      "followerId"
    );
    ...
    const userInfo = await userModel.findOne(
      { _id: shopInfo.owner._id },
      "-_id status lastLogin avatarUrl username"
    );

    return res
      .status(200)
      .send({
        followerInfo: followerInfo.length,
        shopInfo,
        productInfo: productInfo.length,
        rateInfo: rateInfo.length,
        userInfo,
      });
  } catch (err) {
    console.log(err);
