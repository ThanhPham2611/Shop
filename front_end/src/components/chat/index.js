...
import { Affix, Avatar, Col, Dropdown, Empty, FloatButton, Input, Row, Spin } from 'antd';
import moment from 'moment';
import { useSelector } from 'react-redux';
import ramdomstring from 'randomstring';

import { BsFillChatRightTextFill, BsArrowBarRight } from 'react-icons/bs'
import { AiOutlineDownSquare, AiOutlineDown, AiOutlineSend, AiOutlineUser } from 'react-icons/ai'
...
const ChatComponent = () => {
  ...
  const handleRoomChat = (room) => {
    const selectItem = arrayUser.find(item => item.roomId === room.roomId && item.id === room.id);
    ...
    <Affix offsetBottom={0}>
      <Row justify='end'>
        <div className={styles.containerChat}>
          <Row className={styles.headerChat} justify='space-between' align='middle'>
            <Col>
            ...
            <Col xxl={9} className={styles.contentChat}>
              ...
              {arrayUser?.map(item => (
                <Row
                  ...
                  style={{ backgroundColor: item.roomId === highlight.roomId ? 'rgba(0,0,0,.08)' : 'none' }}
                ...
                >
                  ...
                  <Col xxl={19} >
                    <Row justify='space-between' align='middle'>
                      <div className={styles.nameChat}>{item?.user?.username}</div>
                      <span className={styles.timeChat}>{item?.createdAt ? moment(item?.createdAt).fromNow() : ''}</span>
                    </Row>
                    <div className={styles.contentChatbox}>{item?.message || ''}</div>
                  </Col>
                </Row>
              ))}
            </Col>
            {highlight ? (
              <Col xxl={15} className={styles.textChatBox}>
                <div className={styles.headerRightChatBox}>
                  <span className={styles.chatUserName}>{highlight?.user?.username}</span>
                </div>

                {loadingChat ? <Spin style={{ height: '75%', display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }} tip='Loading' /> : (
                  <div className={styles.contentTextChatBox}>
                    {message?.length > 0 ? message?.map(mess => {
                      if (mess.from === _id) {
                        return <p className={styles.fromChat}>{mess.message}</p>
                      } else {
                        return <p className={styles.toChat}>{mess.message}</p>
                      }
                    }) : <Empty />}
                  </div>
                )}

                <Row className={styles.inputChatBox} align='middle' justify='space-between'>
                  <Input
